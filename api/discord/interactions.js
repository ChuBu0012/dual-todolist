...
export default async function handler(req, res) {
    ...
    try {
        ...
        const isVerified = nacl.sign.detached.verify(Buffer.from(timestamp + rawBody), Buffer.from(signature, 'hex'), Buffer.from(publicKey, 'hex'));
        if (!isVerified) {
        ...
        if (payload.type === 2) {
            ...
            let defaultAssignee = 'both';
            if (discordUserId === process.env.DISCORD_MOST_ID)
                defaultAssignee = 'most';
            else if (discordUserId === process.env.DISCORD_FERN_ID)
                defaultAssignee = 'fern';
            if (data.name === 'todo') {
                const itemsOpt = data.options?.find((o) => o.name === 'items');
                const cardOpt = data.options?.find((o) => o.name === 'card');
                const assignOpt = data.options?.find((o) => o.name === 'assign');
                const rawItemsString = itemsOpt?.value || '';
                const cardSearch = cardOpt?.value || 'Inbox';
                const assign = assignOpt?.value || defaultAssignee;
                ...
                let targetCard = cards.find(c => c.title.toLowerCase().includes(cardSearch.toLowerCase()));
                const newItems = itemNames.map((text) => ({
